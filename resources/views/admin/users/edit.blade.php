...
<form method="POST" action="/admin/user/{{$user->id}}" class="form-horizontal" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <fieldset >
  ...
  <div class="mb-3">
    ...
    <input type="text" placeholder="User name" id="name" name="name" value="{{ $user->name }}" class=" form-control {{ $errors->has('title') ? 'error' : '' }}" />
  </div>
  <div class="mb-3">
